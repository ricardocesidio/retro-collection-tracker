...
export class ExternalGamesService {
  ...
  private async searchWikipedia(query: string): Promise<ExternalGameResult[]> {
    ...
    const pageIds = results.slice(0, 10).map((r: any) => r.pageid).join('|');
  ...
  async import(source: string, sourceId: string): Promise<{ id: string; title: string }> {
    const existing = await this.prisma.game.findFirst({
      where: { title: `__ext_${source}_${sourceId}` },
    });
    if (existing) return { id: existing.id, title: existing.title.replace(/^__ext_[a-z]+_/, '') };
    ...
    if (!details.title) throw new BadRequestException('Could not fetch game details');
  ...
  private async importFromRawg(sourceId: string): Promise<Partial<ExternalGameResult>> {
  ...
  private async importFromWikipedia(sourceId: string): Promise<Partial<ExternalGameResult>> {
  ...
  private async resolvePlatform(name: string): Promise<string | undefined> {
    ...
    const existing = await this.prisma.platform.findFirst({
      where: { OR: [{ name: { contains: name, mode: 'insensitive' } }, { slug }] },
    });
  ...
  private async getDefaultPlatform() {
    const p = await this.prisma.platform.findFirst({ orderBy: { name: 'asc' } });
