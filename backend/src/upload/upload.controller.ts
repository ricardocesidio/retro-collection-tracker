...
export class UploadController {
  ...
  async uploadAvatar(@Request() req: any, @UploadedFile() file: Express.Multer.File) {
    ...
    const url = await this.upload.upload(file);
  ...
  @UseGuards(JwtAuthGuard)
  @UseInterceptors(imageInterceptor)
  async uploadGameCover(@Param('gameId') gameId: string, @UploadedFile() file: Express.Multer.File) {
    if (!file) throw new BadRequestException('No file uploaded');
  ...
  @UseGuards(JwtAuthGuard)
  @UseInterceptors(imageInterceptor)
  async uploadCollectionCover(@Param('colId') colId: string, @Request() req: any, @UploadedFile() file: Express.Multer.File) {
    if (!file) throw new BadRequestException('No file uploaded');
