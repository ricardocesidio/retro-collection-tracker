...
export class StatsService {
  ...
  async getCollectionStats(userId: string) {
    ...
    for (const item of platformAgg) {
      ...
      if (existing) existing.count += item._count.gameId;
        ...
        platMap.set(p.id, {
          id: p.id,
          name: p.name,
          count: item._count.gameId,
        });
    }
    const platformDistribution = Array.from(platMap.values())
      .map((p) => ({
        ...p,
        percentage: Math.round((p.count / totalGames) * 100),
      }))
    ...
    const genreDistribution = Array.from(genreMap.values())
      .map((g) => ({
        ...g,
        percentage: Math.round((g.count / totalGames) * 100),
      }))
      .sort((a, b) => b.count - a.count);
    ...
    return {
      summary: {
        totalGames,
        totalValue:
          Math.round((totalValueResult._sum.estimatedValue || 0) * 100) / 100,
        avgRating: totalValueResult._avg.personalRating
